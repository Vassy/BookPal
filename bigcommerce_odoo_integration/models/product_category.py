                ...
                for category_id in category_ids:
                    ...
                    if response_data.status_code in [200, 201]:
                        ...
                        if response_data.get('data') and response_data.get('data').get("id"):
                            ...
                            process_message="Category Id Not Found!"
            ...
            try:
                ...
                if response_data.status_code in [200, 201]:
                    ...
                    for category_response_page in category_response_pages:
                        for record in category_response_page:
                            category_id = self.env['product.category'].search(
                                [('bigcommerce_product_category_id', '=', record.get('id')),('bigcommerce_store_id','=',bigcommerce_store_id.id)], limit=1)
                            if not category_id:
                                vals = {
                                ...
                                req_data = record
                            self.create_bigcommerce_operation_detail('product_category', 'import', req_data,
                    ...
                    category_operation_id and category_operation_id.write(
                        {'bigcommerce_message': category_process_message})
                    self.set_category_parent_id()
                    ...
                    self.create_bigcommerce_operation_detail('product_category', 'import', req_data, response_data,
                                                             category_operation_id, warehouse_id, True, process_message)
            except Exception as e:
                category_process_message = "Process Is Not Completed Yet! %s" % (e)
                _logger.info("Getting an Error In Import Product Category Response {}".format(e))
                process_message = "Getting an Error In Import Product Category Response {}".format(e)
                self.create_bigcommerce_operation_detail('product_category', 'import', response_data,
                                                         category_process_message, category_operation_id, warehouse_id,
                                                         True, process_message)
            category_operation_id and category_operation_id.write({'bigcommerce_message': category_process_message})
            bigcommerce_store_ids.bigcommerce_operation_message = " Import Product Categories Process Complete "
            self._cr.commit()

    def set_category_parent_id(self):
        """
        set parent id of categories
        :return:
        """
        # product categories
        for category in self.search([('bigcommerce_product_category_id', '!=', False)]):
