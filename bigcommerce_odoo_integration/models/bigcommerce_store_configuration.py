...
class BigCommerceStoreConfiguration(models.Model):
    ...
    def bigcommerce_to_odoo_import_orders_main(self,from_date=False,to_date=False):
        ...
        with api.Environment.manage(), db_registry.cursor() as cr:
            env_thread1 = api.Environment(cr, SUPERUSER_ID, self._context)
            t = Thread(target=self.bigcommerce_to_odoo_import_orders, args=(from_date,to_date))
            t.start()

    def bigcommerce_to_odoo_import_orders(self,from_date=False,to_date=False):
        with api.Environment.manage():
            new_cr = registry(self._cr.dbname).cursor()
            self = self.with_env(self.env(cr=new_cr))
            sale_order_obj = self.env['sale.order']
            import_order = sale_order_obj.with_user(1).bigcommerce_to_odoo_import_orders(self.warehouse_id, self,last_modification_date=from_date,today_date=to_date)
            return import_order
