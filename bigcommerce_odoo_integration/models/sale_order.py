...
class SaleOrderVts(models.Model):
    ...
    def update_order_from_bc_to_odoo(self):
        req_data = False
        process_message = "Process Completed Successfully!"
        operation_id = self.with_user(1).create_bigcommerce_operation('order', 'update', self.bigcommerce_store_id,
        ...
        try:
            ...
            todaydate = today_date.strftime("%Y-%m-%d")
            if not self.big_commerce_order_id:
                raise ValidationError("This is Not Bigcommerce Order. We Only Update the Bigcommerce Order")
            api_operation = "/v2/orders/{0}".format(self.big_commerce_order_id)
            response_data = self.bigcommerce_store_id.with_user(1).send_get_request_from_odoo_to_bigcommerce(
                api_operation)
            if response_data.status_code in [200, 201]:
                ...
                try:
                    product_details = "/v2{0}".format(order.get('products').get('resource'))
                    response_data = self.bigcommerce_store_id.send_get_request_from_odoo_to_bigcommerce(product_details)
                    if response_data.status_code in [200, 201, 204]:
                        response_data = response_data.json()
            ...
            _logger.info("Getting an Error In Update Order Response {}".format(e))
            process_message = "Getting an Error In Umport Order Response {}".format(e)
            self.with_user(1).create_bigcommerce_operation_detail('order', 'update', '', '', operation_id,
                                                                  self.warehouse_id,
            ...
            if response_data.status_code in [200, 201]:
                ...
                if not sale_order:
                    ...
                    try:
                        ...
                        if carrier_id and float(base_shipping_cost) > 0:
                            order_id.with_user(1).set_delivery_line(carrier_id, float(base_shipping_cost))
                    except Exception as e:
                        process_message = "Getting an Error In Create Order procecss {}".format(e)
                        self.with_user(1).create_bigcommerce_operation_detail('order', 'import', '', '', operation_id,
                                                                              warehouse_id, True, process_message)
    ...
    def exportordertobigcommerce(self):
        ...
        for line in self.order_line.filtered(lambda line:not line.is_delivery and line.product_id.id != vat_product_id.id):
            ...
            ls.append(data)
        discount = sum(self.order_line.filtered(lambda line: line.price_unit < 0).mapped('price_unit')) or 0.0
        request_data = {
            'status_id': 1,
            'billing_address': {
    ...
    def update_order_from_odoo_to_bc(self):
        ...
        if not response_data.get('id'):
            ...
            self.create_bigcommerce_operation_detail('order', 'export', req_data, response_data,
                                                     ...
                                                     process_message)
        self.big_commerce_order_id = response_data.get('id')
        self.message_post(body="Successfully Order Update from odoo To Bigcommerce")
        return {
            'effect': {
                'fadeout': 'slow',
