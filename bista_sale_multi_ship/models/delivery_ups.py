...
class ProviderUPS(models.Model):
    ...
    def ups_send_shipping(self, pickings):
        res = []
        superself = self.sudo()
        srm = UPSRequest(self.log_xml, superself.ups_username, superself.ups_passwd,
                         superself.ups_shipper_number, superself.ups_access_number, self.prod_environment)
        ...
        for picking in pickings:
            ...
            if picking.package_ids:
                ...
                for package in picking.package_ids:
                    packages.append(Package(
                        self, package.shipping_weight, quant_pack=package.package_type_id, name=package.name))
                    package_names.append(package.name)
            # Create one package with the rest (the content that is not in a
            # package)
            ...
            if self.ups_bill_my_account:
                # custom code for set third_party_account start
                ups_carrier_account = picking.sale_id.third_party_shipper
                # custom code for set third_party_account end
            ...
            check_value = srm.check_required_value(
                picking.company_id.partner_id, picking.picking_type_id.warehouse_id.partner_id, picking.partner_id, picking=picking)
            if check_value:
    ...
    def ups_get_return_label(self, picking, tracking_number=None, origin_date=None):
        ...
        if self.ups_bill_my_account:
            # custom code for set third_party_account start
            ups_carrier_account = picking.shipping_partner_id.with_company(
                picking.company_id).property_ups_carrier_account
            # custom code for set third_party_account end
        if picking.carrier_id.ups_cod:
            cod_info = {
                'currency': picking.partner_id.country_id.currency_id.name,
        ...
        check_value = srm.check_required_value(
            picking.partner_id, picking.partner_id, picking.picking_type_id.warehouse_id.partner_id)
        if check_value:
            raise UserError(check_value)
        ...
        logmessage = _("Return label generated<br/>"
                       ...
                       "<b>Packages:</b> %s") % (carrier_tracking_ref, ','.join(package_names))
        if self.ups_label_file_type != 'GIF':
            attachments = [('%s-%s-%s.%s' % (self.get_return_label_prefix(), pl[0], index,
                                             self.ups_label_file_type), pl[1]) for index, pl in enumerate(package_labels)]
        if self.ups_label_file_type == 'GIF':
            attachments = [('%s-%s-%s.%s' % (self.get_return_label_prefix(), package_labels[
                            0][0], 1, 'pdf'), pdf.merge_pdf([pl[1] for pl in package_labels]))]
        picking.message_post(body=logmessage, attachments=attachments)
        shipping_data = {
            'exact_price': price,
            'tracking_number': carrier_tracking_ref}
