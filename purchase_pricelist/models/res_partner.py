...
class ResPartner(models.Model):
    ...
    def onchange_property_purchase_currency_id(self):
        if self.property_purchase_currency_id != self.property_product_vendor_pricelist.currency_id and self.property_product_vendor_pricelist:
            self.property_product_vendor_pricelist = False
            return {
                'warning': {
                    'title': "Vendor Pricelist Change",
                    'message':"Vendor pricelist have been reset due to currency change."
                }
            }
