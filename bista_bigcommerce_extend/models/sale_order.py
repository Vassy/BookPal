...
class SaleOrderVts(models.Model):
            ...
            try:
                        ...
                        if response_data.status_code in [200, 201]:
                            ...
                            _logger.info(
                                "Order Response Data : {0}".format(
                                    response_data))
                            if self._context.get('import_wizard_id'):
                                response_data = [response_data]
                if self._context.get('import_wizard_id'):
                    import_wizard_id = self._context.get(
                        'import_wizard_id')
                    api_operation = "/v2/orders/{0}".format(
                        import_wizard_id.bc_order_id)
                    response_data = bigcommerce_store_id.\
                        send_get_request_from_odoo_to_bigcommerce(
                            api_operation)
                    if response_data.status_code == 204:
                ...
                if not sale_order:
                    ...
                    carrier_id = self.env['delivery.carrier'].search(
                        [('is_bigcommerce_shipping_method', '=', True)],
                        limit=1)
                    partner_obj = self.env['res.partner'].\
                        ...
                        source_page=1, destination_page=1,
                    ...
                    self.create_product_order_line(
                        ...
                        req_data)
                    if carrier_id and order_id and \
                            float(base_shipping_cost) > 0:
    ...
    def get_odoo_product_id(
            self, bigcommerce_store_id, product_bigcommerce_id, order_line):
    ...
    def prepare_sale_order_lines(
        ...
        for order_line in product_details:
            ...
            product_id = self.get_odoo_product_id(
                bigcommerce_store_id, product_bigcommerce_id, order_line)
            if not product_id:
            ...
            if not sale_order_line:
                ...
                _logger.info("Sale Order line Created".format(
                    sale_order_line and sale_order_line.product_id and
                    sale_order_line.product_id.name))
                response_msg = "Sale order line created %s" % \
                    sale_order_line.product_id.name
                self.create_bigcommerce_operation_detail(
                    'order', 'import', '', '',
                    operation_id, warehouse_id, False,
                ...
                _logger.info("Sale Order line Updated".format(
                    sale_order_line and sale_order_line.product_id and
                    sale_order_line.product_id.name))
                response_msg = "Sale order line updated %s" % \
                    sale_order_line.product_id.name
                self.create_bigcommerce_operation_detail(
        ...
        if coupon_sale_order:
            ...
            for data in coupon_sale_order:
                ...
                if coupon_line:
                    ...
                    response_msg = "Coupon Sale Order line Updated For Order"\
                        "  : {0}".format(coupon_product.name)
                    self.create_bigcommerce_operation_detail(
                        'order', 'update', '', data, operation_id,
                        warehouse_id,
                        False,
                        response_msg)
                else:
                    coupon_line = self.env['sale.order.line'].create(
                        coupon_vals)
                    response_msg = "Coupon Sale Order line Created For Order"\
                        "  : {0}".format(coupon_product.name)
                    self.create_bigcommerce_operation_detail(
