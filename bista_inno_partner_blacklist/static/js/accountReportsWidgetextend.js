odoo.define('bista_inno_extend.accountReportsWidgetextend', function (require) {
    'use strict';
    ...
    // var accountReportsWidget = require('@account_reports/static/src/js/account_reports');
    var accountReportsWidget = require('account_reports.account_report');
    console.log(BaseImport.accountReportsWidget,"xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx",accountReportsWidget)
    debugger
    var QWeb = core.qweb;
    var _t = core._t;
    ...
    accountReportsWidget.include({
        render_searchview_buttons: function() {
            ...
            this.$searchview_buttons.find('.js_account_report_bool_filter').click(function (event) {
                ...
                if (option_value === 'unfold_all') {
                    self.report_options.unfold_all_currency = false
                    self.unfold_all(self.report_options[option_value]);
                }
                if (option_value === 'unfold_all_currency'){
                    self.unfold_currency(self.report_options[option_value]);
                }
                debugger;
                self.reload();
            });
        ...
        unfold_currency: function(bool) {
            var self = this;
            var lines = this.$el.find('.o_account_reports_level1 .js_account_report_foldable');
