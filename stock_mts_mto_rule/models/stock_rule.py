...
class StockRule(models.Model):
    ...
    mts_rule_id = fields.Many2one(
        "stock.rule", string="MTS Rule", check_company=True)
    mto_rule_id = fields.Many2one(
        "stock.rule", string="MTO Rule", check_company=True)
        ...
        for rule in self:
            if rule.action == "split_procurement":
                if not rule.mts_rule_id or not rule.mto_rule_id:
                    msg = _(
                        "No MTS or MTO rule configured on procurement "
                        "rule: %s!") % (rule.name,)
    ...
    def get_mto_qty_to_order(self, product, product_qty, product_uom, values):
        """Updated logic when update po qty with negetive qty."""
        self.ensure_one()
        precision = self.env["decimal.precision"].precision_get(
            "Product Unit of Measure"
        )
        src_location_id = self.mts_rule_id.location_src_id.id
        # find the reserved moves and update the prodcut_qty."""
        dom = [('product_id', '=', product.id),
               ('location_id', '=', src_location_id),
               ('date', '=', values.get('date_planned')),
               ('date_deadline', '=', values.get('date_planned')),
               ('state', '=', 'assigned')]
        reserved_moves = self.env['stock.move'].sudo().search(dom)
        if reserved_moves and product_qty < 0:
            # reserved_moves._do_unreserve()
            for rmv in reserved_moves:
                product_qty += rmv.product_uom_qty
    ...
    def _run_split_procurement(self, procurements):
        ...
        for procurement, rule in procurements:
            ...
            if float_is_zero(needed_qty, precision_digits=precision):
                getattr(self.env["stock.rule"], "_run_%s" %
                        rule.mts_rule_id.action)(
                ...
                # Search all confirmed stock_moves of mts_procuremet and
                # assign the to adjust the product's free qty
