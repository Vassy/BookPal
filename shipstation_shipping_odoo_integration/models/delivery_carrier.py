...
class DeliveryCarrier(models.Model):
    ...
    def create_or_update_order(self, picking):
        ...
        picking_receiver_id = picking.partner_id
        company = picking.partner_id.parent_id.name if picking.partner_id.parent_id else ""
        picking_sender_id = picking.picking_type_id.warehouse_id.partner_id
        total_value = picking.sale_id.amount_total if picking.sale_id.amount_total else sum(
            [(line.product_uom_qty * line.product_id.list_price) for line in picking.move_lines]) or 0.0
        total_tax = picking.sale_id.amount_tax or 0.0
        ...
        if date_order:
            ...
            request_data = {
                "orderNumber": "%s" % (origin),
                "orderDate": "%s" % (order_date),
                "shipByDate": "%s" % (order_date),
                "orderStatus": "awaiting_shipment",
                "customerUsername": "%s" % (picking.sale_id.partner_id.name if picking.sale_id else picking.partner_id.name),
    ...
    def shipstation_cancel_shipment(self, picking):
        shipment_ids = picking.shipstation_shipment_id
        if not shipment_ids:
            raise ValidationError("Shipstation Shipment Id Not Available!")
        for shipment_id in shipment_ids.split(','):
